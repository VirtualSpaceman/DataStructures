...
class AvlTree:
    ...
    # funcao auxiliar somente para gerar um grafo no graphviz
    def bfs(self):
        self.tree_dict = dict()
        q = deque()
        if self.root:
    ...
    # traduz o grafo montando pela bfs e cria o arquivo do graphviz para ser renderizado

    def show(self, operation):
        self.bfs()
        avl = Digraph( format='png', node_attr={'shape': 'record', 'height': '.1'})
        ...
        avl.render(filename='avl_graph', view=False, directory='./modules/AVL/graphviz_avl')
                ...
                if self.height == 0:
                    ...
                    if right_tree.root and left_tree.root:
                        if left_tree.height >= right_tree.height:
                            left_tree.reinsertion(right_tree)
                            self.__dict__.update(right_tree.__dict__)
                        else:
                            right_tree.reinsertion(left_tree)
    ...
    def simple_left_right_rotation(self):
        print('simple_left_right_rotation at', self.root.key)
    ...
    def simple_right_left_rotation(self):
        print('simple_right_left_rotation at', self.root.key)
    ...
    def double_left_right_rotation(self):
        print('double_left_right_rotation', self.root.key)
    ...
    def double_right_left_rotation(self):
        print('double_right_left_rotation', self.root.key)
